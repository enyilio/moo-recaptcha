<?php
/**
 * Plugin Name: Moospace reCAPTCHA
...
function moo_recaptcha_verify( $token = null ) {
    ...
    if ( empty( $o['secret_key'] ) ) {
        ...
        if ( is_admin() && current_user_can( 'manage_options' ) ) {
            add_action( 'admin_notices', function() {
                echo '<div class="notice notice-error"><p><strong>Moospace reCAPTCHA：</strong>Secret Key 尚未設定，所有表單驗證已停用。請至「設定 → Moo reCAPTCHA」填寫金鑰。</p></div>';
            });
        }
        return true;
    }
    ...
    if ( is_wp_error( $response ) ) {
        error_log( 'Moospace reCAPTCHA: Google API unreachable - ' . $response->get_error_message() );
